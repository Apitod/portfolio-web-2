...
type ImageItem = string | { src: string; alt?: string; description?: string; type?: 'image' | 'video' };
...
function buildItems(pool: ImageItem[], seg: number): ItemDef[] {
  ...
  if (pool.length === 0) {
    return coords.map(c => ({ ...c, src: '', alt: '', type: 'image' }));
  }
  ...
  return coords.map((c, i) => ({
    ...
    type: usedImages[i].type as 'image' | 'video'
  }));
}
...
export default function DomeGallery({
  ...
  const openItemFromElement = (el: HTMLElement) => {
    ...
    if (description) {
      ...
      descDiv.className = 'enlarge-description';
    ...
    setTimeout(() => {
      ...
      // Animate description
      const desc = overlay.querySelector('.enlarge-description');
      if (desc) {
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      if (originalMedia) {
        const media = originalMedia.cloneNode() as HTMLElement;
